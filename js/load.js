function LoadMyOrder() {
        ...
        if (this.readyState == 4 && this.status == 200) {
            if (this.responseText) {
                ...
                for (var i = 0; i < data.length; i++) {
                    if (document.getElementById("MyOrderstatus").value == "All" || document.getElementById("MyOrderstatus").value == data[i]['order_status']) {
                        ...
                        if (data[i]['order_status'] == "undone") {
                            ...
                            row9.innerHTML = '<button type="button" class="btn btn-danger" onclick=CancelOrder([' + parseInt(data[i]["OID"]) + '])>Cancel</button>';
                        ...
                        var detail = JSON.parse(data[i]["order_detail"]);
    ...
    xhttp.open("POST", "php/load_MyOrder.php", true);
...
function LoadShopOrder() {
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            if (this.responseText) {
                ...
                var idx = 0;
                for (var i = 0; i < data.length; i++) {
                    if (document.getElementById("ShopOrderstatus").value == "All" || document.getElementById("ShopOrderstatus").value == data[i]['order_status']) {
                        let row1 = document.createElement('tr');
                        let row2 = document.createElement('th');
                        let row3 = document.createElement('td');
                        let row4 = document.createElement('td');
                        ...
                        if (data[i]['order_status'] == "undone") {
                            ...
                            row9.innerHTML = '<button type="button" class="btn btn-success"  onclick=FinishOrder([' + parseInt(data[i]["OID"]) + '])>Done</button>';
                            row10.innerHTML = '<button type="button" class="btn btn-danger" onclick=CancelOrder([' + parseInt(data[i]["OID"]) + '])>Cancel</button>';
                        ...
                        for (var j = 1; j < detail.length; j++) {
                            tbody += "<tr>";
                            tbody += "<th scope='row'>"+ detail[j]['picture'] +" </th>";
                            tbody += "<td>" + detail[j]["meal"] + "</td>";
                            tbody += "<td>" + detail[j]["price"] + "</td>";
                            tbody += "<td>" + detail[j]["quantity"] + "</td>";
                            tbody += "</tr>";
    ...
    xhttp.open("POST", "php/load_ShopOrder.php", true);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send();
}
...
function LoadTransactionRecord() {
    ...
    xhttp.open("POST", "php/load_TransactionRecord.php", true);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send("tra_action=" + document.getElementById("TransactionRecordstatus").value);
}